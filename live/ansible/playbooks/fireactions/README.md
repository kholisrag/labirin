...
| fireactions-03 | `BC:24:11:88:AB:24` | `10.10.99.15` | `fireactions-03.internal.khol.is` |
| fireactions-04 | `BC:24:11:88:AB:25` | `10.10.99.16` | `fireactions-04.internal.khol.is` |
...
> [!NOTE]
> `opnsense_host` in that vault is `opnsense.khol.is`. A workstation whose
> `/etc/resolver/` only covers `internal.khol.is` cannot resolve it, and every
...
> echo 'nameserver 192.168.3.102' | sudo tee /etc/resolver/khol.is
> ```
>
...
`harbor.internal.khol.is/ghcr/oprek-sh/fireactions-images/ubuntu24.04:TAG`,
...
[`oprek-sh/fireactions-images`](https://github.com/oprek-sh/fireactions-images)
...
- **`/etc/resolv.conf` pointing at `10.10.1.1`.** The upstream image bakes
  `1.1.1.1` and `8.8.8.8`, so **no microVM could resolve `*.internal.khol.is`** —
  which is why Athens was configured correctly for days and served nothing while
  CI stayed green. Measured in
  [oprek-sh/oprek.sh#133](https://github.com/oprek-sh/oprek.sh/issues/133).
- **A patched `Runner.Worker.dll`**, from `-a1abf5a` onward, so `actions/cache`
  reaches `actions-cache.internal.khol.is` instead of GitHub. The shim is not
  ...
  [oprek-sh/oprek.sh#131](https://github.com/oprek-sh/oprek.sh/issues/131).
...
>
> ```bash
> curl -s https://actions-cache.internal.khol.is/metrics | grep '^cache_'
> ```
...
> `/v2/ghcr/oprek-sh/fireactions-images/ubuntu24.04/manifests/<tag>` returns 200.
>
> But a **direct** `ghcr.io/oprek-sh/...` reference would fail, and that is what
