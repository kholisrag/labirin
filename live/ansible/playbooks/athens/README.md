...
after that. It also fetches the private `github.com/oprek-sh/*` modules on
...
| 1Panel website + OpenResty | TLS termination for `goproxy.internal.khol.is` |
...
├── vars/main.yaml                       # the goproxy.internal.khol.is entry
...
```text
  runner microVM / workstation
        |  GOPROXY=https://goproxy.internal.khol.is
        v
  ...
  athens container, same VM, published on loopback only
        ...
        +--> github.com              (github.com/oprek-sh/*, via the token)
```
...
| --- | --- | --- |
| `goproxy.internal.khol.is` | Unbound **alias** → `1panel.internal.khol.is` | Every client. Nothing else. |
...
`unbound_host_aliases`, pointing `goproxy` at `1panel.internal.khol.is` — the
...
credentials, `/github.com/oprek-sh/oprek.sh/@v/list` returns 404 while
...
> `https://goproxy.internal.khol.is` can pull any module this credential can
...
> token rotation. An App installed only on `oprek-sh` *cannot* read the other
...
```bash
curl -fsS https://goproxy.internal.khol.is/readyz          # storage is writable
curl -fsS https://goproxy.internal.khol.is/version         # build info
...
GOPROXY=https://goproxy.internal.khol.is GONOSUMDB='github.com/oprek-sh/*' \
  go list -m github.com/oprek-sh/oprek.sh@latest
```
...
```bash
go env -w GOPROXY=https://goproxy.internal.khol.is,direct
go env -w GONOSUMDB='github.com/oprek-sh/*'
```
