...
7. **Install App** on both:
   - the `<org>` organisation → serves the `small` / `medium` / `large` tiers
...
| --- | --- | --- | --- | --- | --- | --- |
| small | `fireactions-small` | `<org>` org | 2 | 4 GiB | 1 | 4 GiB |
| medium | `fireactions-medium` | `<org>` org | 4 | 8 GiB | 1 | 8 GiB |
| large | `fireactions-large` | `<org>` org | 8 | 12 GiB | 1 | 12 GiB |
...
`<org>` repository can only see the org runners; one in `kholisrag/labirin`
...
- org tiers → <https://github.com/organizations/<org>/settings/actions/runners>
...
- Runners never appear in GitHub — check the App is *installed* on that pool's
  scope, not just created. Installing it on `<org>` does nothing for
