locals {
  ...
  # ------------------------------------------------------------------
  # oprek.sh's CI has two tiers, split by what can host /dev/kvm rather than by
  # speed (oprek.sh ADR-0035):
  #
  ...
  # Firecracker cannot expose VMX/SVM to its guest (oprek.sh ADR-0021). So a
  ...
  # eliminate it, because a breakout still lands on petruk-pve0, which oprek.sh
...
inputs = {
  vms = { for name, host in local.hosts : name => merge(local.vm_defaults, {
    name        = name
    vm_id       = host.vm_id
    description = format("oprek.sh Tier 1 CI runner - the only runner with /dev/kvm (%s)", name)
