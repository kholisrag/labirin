...
### 2. GitHub App on the `<org>` organisation
...
6. Create the App, note the **App ID**, then **Generate a private key** and keep
   the downloaded `.pem`.
7. **Install App** → install it on the `<org>` organisation.
...
<https://github.com/organizations/<org>/settings/actions/runners>. Then run a
