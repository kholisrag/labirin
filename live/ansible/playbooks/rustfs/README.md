...
```text
  aws-cli / SDK / browser
        |  https://rustfs.internal.khol.is           (S3 API)
        |  https://rustfs-console.internal.khol.is   (console)
        v
...
| --- | --- | --- |
| `rustfs.internal.khol.is` | Unbound **alias** → `1panel.internal.khol.is` | Every S3 client. This is the endpoint URL. |
| `rustfs-console.internal.khol.is` | Unbound **alias** → `1panel.internal.khol.is` | The web console, in a browser. |
| `rustfs-01.internal.khol.is` | **A record** → `10.10.99.14` | SSH, Ansible, and the 1Panel upstream. Not an S3 endpoint. |
...
`https://rustfs.internal.khol.is/<bucket>/<key>` and **not** as
`https://<bucket>.rustfs.internal.khol.is/<key>`.

The blocker is the certificate, not RustFS: the Let's Encrypt wildcard OPNsense
renews covers `*.internal.khol.is`, which matches exactly **one** label, so
`mybucket.rustfs.internal.khol.is` is a level too deep and does not verify.
Enabling virtual-host style needs a `*.rustfs.internal.khol.is` wildcard issued
...
| `rustfs-01.internal.khol.is` | `unbound_hosts` | `10.10.99.14` |
| `rustfs.internal.khol.is` | `unbound_host_aliases` | → `1panel.internal.khol.is` |
| `rustfs-console.internal.khol.is` | `unbound_host_aliases` | → `1panel.internal.khol.is` |

```bash
ansible-vault edit live/ansible/playbooks/opnsense/vars/vault.yaml
```

Append to `dnsmasq_hosts` (the `.khol.is` domain here is the DHCP reservation's
own zone, mirroring `harbor.khol.is` — the name clients use is the Unbound alias
further down, not this):

```yaml
  - description: RustFS Object Storage Host 01
    domain: rustfs.khol.is
    hardware_addr:
      - BC:24:11:88:AB:23
    host: rustfs-01
    ip:
...
aws --endpoint-url https://rustfs.internal.khol.is s3 mb s3://scratch
aws --endpoint-url https://rustfs.internal.khol.is s3 cp ./file s3://scratch/
aws --endpoint-url https://rustfs.internal.khol.is s3 ls s3://scratch/
```
...
# From the LAN
curl -fsS https://rustfs.internal.khol.is/health/ready
aws --endpoint-url https://rustfs.internal.khol.is s3 ls
```
...
- **`413 Request Entity Too Large` on an upload** — OpenResty, not RustFS.
  ...
  50m).
- **A client insists on `<bucket>.rustfs.internal.khol.is`** — it is using
