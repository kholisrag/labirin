...
provider "helm" {
  kubernetes = {
    host  = var.private_endpoint ? format("https://%s", data.google_container_cluster.gke.private_cluster_config[0].private_endpoint) : format("https://%s", data.google_container_cluster.gke.endpoint)
...
resource "helm_release" "release" {
  for_each = {
    for i in local.helm_releases : format("%s_%s_%s", var.gke_cluster_location, var.gke_cluster_name, i.name) => i
  }
