...
### 1. Split DNS for `internal.khol.is`

Nothing in this repo resolves without it. Every playbook targets a
`*.internal.khol.is` name served by Unbound on OPNsense, which is not a public
zone.

```bash
# macOS
sudo mkdir -p /etc/resolver
printf 'nameserver 192.168.3.102\n' | sudo tee /etc/resolver/internal.khol.is
```

On systemd-resolved the equivalent is a per-link domain routing rule
(`resolvectl domain <link> '~internal.khol.is'` plus
...
> [!IMPORTANT]
> **This resolver covers `internal.khol.is` and nothing else.** `khol.is` names
> still resolve publicly, and the two differ. The OPNsense vault sets
> `opnsense_host` to `opnsense.khol.is`, which therefore does **not** resolve
...
```bash
dig +short @192.168.3.102 <name>.internal.khol.is   # the truth
